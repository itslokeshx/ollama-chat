...
import { Copy, RefreshCw, ThumbsUp, ThumbsDown, Pencil, Check, X } from 'lucide-react';
...
export function MessageBubble({
  ...
  if (isUser) {
    return (
      <div
        className="flex justify-end group message-animate-in px-4 py-1"
        ...
        <div className="flex flex-col items-end gap-1 max-w-[72%]">
          {message.attachments && message.attachments.length > 0 && (
            <div className="flex flex-wrap gap-2 justify-end">
              {message.attachments.map((att) => (
                <FileChip key={att.id} attachment={att} readonly onImageClick={onImageClick} />
              ))}
            </div>
          )}
          ...
          {editing ? (
            ...
            <div className="relative">
              <div className="bg-primary text-primary-foreground rounded-[18px_18px_4px_18px] px-4 py-3 text-sm whitespace-pre-wrap">
                {message.content}
              </div>
              {hovered && !isStreaming && (
  ...
  return (
    <div
      className="flex justify-start group message-animate-in px-4 py-1"
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
